...
class InventoryEnvironment(gym.Env):
    def __init__(self, initial_bloem_units, initial_ware_units, initial_prod_units, initial_durb_units,
        ...
        self.num_stock_points = 4  # initially just considering bloem, production, warehouse, durb
    ...
    def reset(self, **kwargs):
        ...
        obs = [self.prod_units_sum, self.ware_units_sum, self.bloem_units_sum, self.durb_units_sum,
               self.choice_bloem, self.choice_durb, self.no_of_trucks_prod_ware,
               self.no_of_trucks_prod_bloem, self.no_of_trucks_ware_bloem, self.no_of_trucks_prod_durb,
               self.no_of_trucks_ware_durb,
               self.units_transit_prod_ware, self.units_transit_prod_bloem, self.units_transit_ware_bloem,
               self.units_transit_prod_durb, self.units_transit_ware_durb]
    ...
    def step(self, action):
        ...
        # 5) Apply customer fulfilment at bloem
        ...
        # 6) Apply customer fulfilment at durb
        ...
        # 7) Apply customer fulfilment at ware
        ...
        # 9) reduce product lifespan by 1 and check for obsolete
        for i in range(len(self.prod_units)):
        ...
        # 10) storage costs for remaining inventory
        ...
        obs = [self.prod_units_sum, self.ware_units_sum, self.bloem_units_sum, self.durb_units_sum,  self.choice_bloem,
               self.choice_durb, self.no_of_trucks_prod_ware, self.no_of_trucks_prod_bloem, self.no_of_trucks_ware_bloem,
               self.no_of_trucks_prod_durb, self.no_of_trucks_ware_durb,
               self.units_transit_prod_ware, self.units_transit_prod_bloem, self.units_transit_ware_bloem, self.units_transit_prod_durb,
               self.units_transit_ware_durb]
        bloem_forecast = self.total_pred_bloem[self.day-5:self.day + 5]
        obs.extend(bloem_forecast)
        ware_forecast = self.total_pred_jhb[self.day-5:self.day + 5]
        obs.extend(ware_forecast)
...
if __name__ == "__main__":
    ...
    with summary_writer.as_default():
        ...
        for day in range(341):
            ...
            # print(f"Obs: {obs}")
            print(f"Prod units {env.prod_units_sum} Ware units {env.ware_units_sum} Bloem units {env.bloem_units_sum} Durb units {env.durb_units_sum}")
            print(f"bloem choice: {env.choice_bloem} durb choice {env.choice_durb}")
